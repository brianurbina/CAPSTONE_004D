...
def finalizar_donacion(request, peticion_id):
    if request.method == 'POST':
        ...
        try:
            ...
            # Verificar si ya existe un donador
            donador = Usuario.objects.get(username=donador_nombre)
            
            # Agregar el donador a la petición
            peticion.donador = donador
            peticion.estado = 'en_progreso'
            peticion.save()

            # Crear una notificación para el donador
            Notificacion.objects.create(
                usuario=donador.username,
                mensaje=f"Has completado la donación para la petición {peticion.titulo}.",
                respuesta="PENDIENTE"
            )
