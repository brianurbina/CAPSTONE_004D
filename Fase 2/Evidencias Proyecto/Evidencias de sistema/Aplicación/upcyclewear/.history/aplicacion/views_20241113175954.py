...
def index(request):
    # Obtener el número de donaciones y peticiones con el estado 'Completada'
    contador_donaciones = Donacion.objects.filter(estado='completada').count()
    contador_peticiones = Peticion.objects.filter(estado='completada').count()
    contador_fundaciones = Fundacion.objects.filter(aprobada=True).count()

    # Obtener las donaciones y peticiones recientes (últimos 5 elementos)
    donaciones_recientes = Donacion.objects.filter(estado='activa').order_by('-fecha_hora')[:4]
    peticiones_recientes = Peticion.objects.filter(estado='activa').order_by('-fecha_hora')[:4]

    donacion_usuario= Donacion.objects.filter(usuario=request.user).count()
    peticion_usuario= Peticion.objects.filter(usuario=request.user).count()
    
    # Pasar el contador y las donaciones/peticiones recientes al contexto
    context = {
        'contador_fundaciones': contador_fundaciones,
        'contador_donaciones': contador_donaciones,
        'contador_peticiones': contador_peticiones,
        'donaciones_recientes': donaciones_recientes,
        'peticiones_recientes': peticiones_recientes,
        'donacion_usuario': donacion_usuario,
...
def register(request):
    if request.method == 'POST':
        ...
        # Verificar si la dirección fue seleccionada desde Google Maps
        ...
        if form.is_valid():
            if not direccion_validada:
                ...
                subject_customer = f'¡Bienvenid@ {user.username}! | Upcycle Wear'
                html_content_customer = render_to_string('emails/nuevacuenta.html', {'user': user})
                ...
                email_customer = EmailMultiAlternatives(
                    ...
                    from_email='upcyclewearcl@gmail.com',
                    to=[user.email],
                )
    ...
    return render(request, 'register/register.html', {
        'form': form,
        'GOOGLE_MAPS_API_KEY': 'AIzaSyAtwlRCttq5JUstF8LC0WohrFXnosyfWiA'  # Agrega tu API key aquí
    })
...
def crear_fundacion(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Fundación creada exitosamente, recibira una respuesta a su correo{fundacion.usuario.email}')
            return render(request, 'menuuser/form_fundacion.html', {'form': form})  # nos redige ahora al forms para ver el mensaje Renderiza el formulario con el mensaje de éxito
