...
def obtener_mensajes_no_leidos(request):
    if request.user.is_authenticated:
        # Filtrar mensajes donde el usuario sea 'usuario1' o 'usuario2' en la conversación
        mensajes_no_leidos = Mensaje.objects.filter(
            (Q(conversacion__usuario1=request.user) | Q(conversacion__usuario2=request.user)),
            leido=False
        )
