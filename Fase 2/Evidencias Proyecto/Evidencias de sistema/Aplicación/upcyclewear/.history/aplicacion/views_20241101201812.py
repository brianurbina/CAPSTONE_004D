...
def actualizar_estado_reporte(request, reporte_id, nuevo_estado):
    if request.method == 'POST':
        try:
            # Cambia 'id' por 'id_reporte'
            reporte = get_object_or_404(ReporteUsuario, id_reporte=reporte_id)  
            data = json.loads(request.body)

            resultado = data.get('resultado', '')  # Cambia 'motivo' por 'resultado'

            reporte.estado = nuevo_estado
            reporte.resultado = resultado  # Asegúrate de que este campo exista en tu modelo
            reporte.save()

            # Enviar correo al usuario reportante
            subject_customer = 'Actualización de tu Reporte'
            html_content_customer = render_to_string('email/actualizacion_reporte.html', {
                'reporte': reporte
            })
            ...
            email_customer = EmailMultiAlternatives(
                ...
                from_email='upcyclewearcl@gmail.com',  # Cambia esto por tu correo
                to=[reporte.usuario_reportante.email],  # Asegúrate de que este campo esté disponible
            )
