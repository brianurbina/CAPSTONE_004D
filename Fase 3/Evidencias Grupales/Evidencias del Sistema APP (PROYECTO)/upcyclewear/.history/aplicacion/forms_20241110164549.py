...
#FILTRO PARA MI DONAR HTML
class FiltroPeticionDonacion(forms.Form):
    # Filtrar las opciones para eliminar 'Seleccione una opción'
    tipo_de_ropa_filtradas = [
        option for option in Peticion.TIPO_ROPA_CHOICES if option[0] != ''
    ]
...
#FILTRO PARA MI PEDIR HTML 
class FiltroPedir(forms.Form):
    # Filtrar las opciones para eliminar 'Seleccione una opción'
    tipo_de_ropa_filtradas = [
        option for option in Donacion.TIPO_ROPA_CHOICES if option[0] != ''
    ]

    # Campo para seleccionar el tipo de ropa con opción "Todos"
    tipo_ropa = forms.ChoiceField(
        choices=[('TODOS', 'Todos')] + tipo_de_ropa_filtradas,  # Agregar 'TODOS' al principio
        required=False,
        widget=forms.Select(attrs={'class': 'form-control', 'id': 'tipo_ropa'})
    ...
    # Campo para seleccionar la talla, con opción "Todas"
    talla = forms.ChoiceField(
        choices=[('', 'Todas')] + Donacion.TALLAS,  # Agregar 'Todas' al principio
        required=False,
        widget=forms.Select(attrs={'class': 'form-control', 'id': 'talla'})
    )

    # Campo para seleccionar la temporada, con opción "Todas"
    temporada = forms.ChoiceField(
        choices=[('', 'Todas')] + Donacion.TEMPORADAS,  # Agregar 'Todas' al principio
        required=False,
        widget=forms.Select(attrs={'class': 'form-control', 'id': 'temporada'})
    )