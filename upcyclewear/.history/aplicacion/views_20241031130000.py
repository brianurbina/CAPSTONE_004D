...
def mi_perfil(request):
    user = request.user  # Asegúrate de que estás utilizando el usuario que ha iniciado sesión
    ...
    if request.method == 'POST':
        ...
        if request.FILES.get('foto_perfil'):
            ...
            user.foto_perfil = None  # Eliminar la foto de perfil, asignar None o la imagen por defecto

        user.save()
        return render(request, 'perfil/mi_perfil.html', {'user': user, 'success_message': 'Perfil actualizado correctamente.'})

    return render(request, 'perfil/mi_perfil.html', {'user': user})
