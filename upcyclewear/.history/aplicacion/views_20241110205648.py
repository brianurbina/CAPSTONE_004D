    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if foto_perfil:
                ...
                image_data = foto_perfil.split(',')[1]
                image_data = base64.b64decode(image_data)

                # Guardar la imagen en un archivo
                image_name = f"{uuid.uuid4()}.png"
                with open(f'path/to/save/{image_name}', 'wb') as f:
                    f.write(image_data)

                # Asignar la imagen al modelo de usuario
                user.foto_perfil = f'path/to/save/{image_name}'
