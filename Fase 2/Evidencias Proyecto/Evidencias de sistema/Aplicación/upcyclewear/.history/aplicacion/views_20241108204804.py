...
def misPeticiones(request, username): 
    ...
    peticiones = Peticion.objects.filter(usuario=user)  # Obtener solo las peticiones de ese usuario
...
@login_required
def finalizar_donacion(request, donacion_id):
    # Obtiene la donación sólo si pertenece al usuario autenticado
    ...
    if request.method == 'POST':
        # Obtiene el receptor seleccionado desde el formulario
        ...
        # Notifica al receptor (esto es un mensaje de ejemplo, puedes configurarlo mejor)
        messages.success(request, f"Notificación enviada al receptor {receptor.username}")

        # Redirige al usuario a su lista de donaciones
        return redirect('misDonaciones', username=request.user.username)

    # Obtiene todos los usuarios, excepto el usuario actual, para ser elegibles como receptores
    receptores = Usuario.objects.exclude(id=request.user.id)

    # Renderiza la página donde se selecciona el receptor
    return render(request, 'perfil/seleccionar_receptor.html', {
        'donacion': donacion,
