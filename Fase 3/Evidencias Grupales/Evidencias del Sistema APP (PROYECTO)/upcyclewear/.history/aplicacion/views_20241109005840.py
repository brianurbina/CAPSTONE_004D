...
# Vista para aceptar la notificación
def aceptar_notificacion(request, notificacion_id):
    notificacion = Notificacion.objects.get(id=notificacion_id)
    notificacion.respuesta = 'SI'
    peticion = Peticion.objects.get(id=notificacion.peticion.id)
    peticion.estado = 'completada'
    peticion.donador = notificacion.usuario
    peticion.save()
    ...
    usuario.total_donaciones += 1  # Incrementa en 1 el total de donaciones
    usuario.save() 
    return redirect(request.META.get('HTTP_REFERER', 'index'))  # Redirigir a donde necesites

# Vista para rechazar la notificación
def rechazar_notificacion(request, notificacion_id):
    notificacion = Notificacion.objects.get(id=notificacion_id)
    peticion = Peticion.objects.get(id=notificacion.peticion.id)
    peticion.estado = 'activa'
    peticion.donador = None
    peticion.save()
...
def finalizar_donacion2(request, id):
    if request.method == 'POST':
        # Obtener los datos enviados
        data = json.loads(request.body)
        receptor_nombre = data.get('receptor')
        
        # Verificar si se ha proporcionado el nombre del donador
        if not receptor_nombre:
            return JsonResponse({'success': False, 'error': 'El nombre del donador es necesario'})
        ...
        try:
            ...
            # Obtener el usuario donador
            receptor = Usuario.objects.get(username=receptor_nombre)  # Primero obtenemos la instancia del Usuario
            
            # Asignar el donador y actualizar el estado de la donación
            donacion.receptor = receptor  # Ahora asignamos el donador como receptor de la donación
            ...
            Notificacion.objects.create(
                id=notificacion_id,
                usuario=receptor,  # Usamos la instancia del donador
                mensaje=f"El usuario {donacion.usuario.username} ha solicitado finalizar la donación: {donacion.titulo}.",
                leida=False,
                respuesta="PENDIENTE",
                peticion=donacion  # Relacionamos la notificación con la donación
            )
