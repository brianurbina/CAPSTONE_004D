...
@csrf_exempt  # Solo si necesitas deshabilitar CSRF para pruebas; lo ideal es mantener CSRF habilitado
def actualizar_estado_reporte(request, reporte_id, nuevo_estado):
    # Verificamos que la solicitud sea POST
    if request.method == 'POST':
        reporte = get_object_or_404(ReporteUsuario, reporte_id=reporte_id)

        # Obtener el motivo del cuerpo de la solicitud JSON
        try:
            data = json.loads(request.body)
            motivo = data.get('motivo', '')

            # Actualizamos el estado y el motivo del reporte
            reporte.estado = nuevo_estado
            reporte.motivo = motivo  # Asegúrate de que 'motivo' sea un campo en tu modelo
            reporte.save()
