...
def finalizar_donacion(request, peticion_id):
    if request.method == 'POST':
        ...
        try:
            donador = Usuario.objects.get(username=donador_username)
            peticion.donador = donador
            peticion.estado = 'En proceso'
            peticion.save()

            # Notificación al usuario
            # Aquí podrías usar un sistema de notificaciones de Django o un sistema personalizado

            return JsonResponse({'success': True, 'message': 'Donación finalizada correctamente.'})

        except Usuario.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'El nombre de usuario donador no es válido.'})
