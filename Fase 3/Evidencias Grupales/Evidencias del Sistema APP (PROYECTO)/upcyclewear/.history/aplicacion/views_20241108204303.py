...
def finalizar_donacion(request, donacion_id):
    ...
    if request.method == 'POST':
        ...
        # Asigna el receptor seleccionado
        ...
        # Enviar la respuesta al receptor (a través de una vista o mediante notificación en el frontend)
...
def buscar_usuarios(request):
    ...
    ).exclude(id=request.user.id)[:10]  # Limitar a 10 resultados para eficiencia
    results = [{'id': usuario.id, 'username': usuario.username} for usuario in usuarios]
