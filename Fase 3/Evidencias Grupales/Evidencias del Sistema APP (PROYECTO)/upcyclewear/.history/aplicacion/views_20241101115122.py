...
def eliminar_fundacion(request, rut):
    ...
    if request.method == 'POST':
        fundacion.delete()
        messages.success(request, 'Fundación eliminada con éxito.')
        return redirect('fundaciones')  # Cambia esto según tu URL

    return render(request, 'confirmar/confirmar_elim_fundacion.html', {'objeto': fundacion, 'tipo': 'Fundación'})
...
def eliminar_donacion(request, id):
    ...
    if request.method == 'POST':
        donacion.delete()
        messages.success(request, 'Donación eliminada con éxito.')
        return redirect('donar')  # Cambia esto según tu URL

    return render(request, 'confirmar/confirmar_elim_donacion.html', {'objeto': donacion, 'tipo': 'Donación'})
...
def eliminar_peticion(request, id):
    ...
    if request.method == 'POST':
        peticion.delete()
        messages.success(request, 'Petición eliminada con éxito.')
        return redirect('pedir')  # Cambia esto según tu URL

    return render(request, 'confirmar/confirmar_elim_peticion.html', {'objeto': peticion, 'tipo': 'Petición'})
...
def solicitud_fundaciones(request):
    ...
    return render(request, 'administrador/solicitud_fundaciones.html', {'foundations': page_obj, 'query': query})
