    ...
    if request.method == 'POST':
        ...
        if form_type == 'profile':  # Actualizar el perfil
            ...
            if request.FILES.get('foto_perfil'):
                ...
                # Si se sube una nueva foto, se actualizará. 
            # Si no se sube una nueva foto, simplemente no se hace nada con user.foto_perfil
...
def gestionar_reportes(request):
    """
    Muestra una lista de reportes para ser gestionados por el moderador.
    """
    reportes = ReporteUsuario.objects.all()  # Filtrar según necesidad
    return render(request, 'reportes/gestionar_reportes.html', {'reportes': reportes})

@csrf_exempt
def actualizar_estado_reporte(request, reporte_id):
    """
    Actualiza el estado de un reporte y envía un correo al usuario reportante con el resultado.
    """
    if request.method == 'POST':
        data = json.loads(request.body)
        ...
        # Obtener el reporte
        reporte = get_object_or_404(ReporteUsuario, reporte_id=reporte_id)  # No hay cambios aquí
        
        # Actualizar el estado del reporte
        reporte.estado = estado
        reporte.save()

        # Enviar correo de notificación al usuario reportante
        asunto = f"Reporte de Usuario - Estado: {estado}"
        mensaje_html = f"""
        <p>Estimado {reporte.usuario_reportante.username},</p>
        <p>Su reporte sobre el usuario <strong>{reporte.usuario_reportado.username}</strong> ha sido <strong>{estado.lower()}</strong>.</p>
        <p><strong>Motivo de la decisión:</strong> {motivo}</p>
        <p>Gracias por ayudarnos a mejorar la comunidad.</p>
        <p>Atentamente,<br>El equipo de moderación</p>
        """

        destinatario = reporte.usuario_reportante.email
        send_mail(asunto, '', 'upcyclewearcl@gmail.com', [destinatario], html_message=mensaje_html)
