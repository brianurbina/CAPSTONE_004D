...
import json
from smtplib import SMTPException  # Importar desde smtplib

def actualizar_estado_reporte(request, reporte_id, nuevo_estado):
    if request.method == 'POST':
        try:
            # Obtener el reporte correspondiente
            reporte = get_object_or_404(ReporteUsuario, id_reporte=reporte_id)
            data = json.loads(request.body)

            resultado = data.get('resultado', '')  # Obtén el resultado del cuerpo de la solicitud

            # Actualizar el estado y resultado del reporte
            ...
            email_customer = EmailMultiAlternatives(
                ...
                from_email='upcycleweaarcl@gmail.com',  # Cambia esto por tu correo
            ...
            return JsonResponse({'status': 'error', 'message': 'Error al enviar el correo: ' + str(e)}, status=500)
        except Exception as e:
