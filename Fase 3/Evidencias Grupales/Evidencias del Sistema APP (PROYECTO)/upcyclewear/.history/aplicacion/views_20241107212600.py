...
def fundaciones(request):
    ...
    if form.is_valid():
        ...
        query = form.cleaned_data.get('query')  # Asegúrate de que el formulario tenga un campo "query"
        
        # Filtrar por comuna_region si se selecciona una comuna específica (distinta de 'TODOS')
        if comuna_region and comuna_region != 'TODOS':
            foundations = foundations.filter(comuna_region=comuna_region)

        # Filtrar por consulta de búsqueda solo si 'query' tiene texto
        if query:
