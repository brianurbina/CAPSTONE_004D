...
def mi_perfil(request):
    ...
    if request.method == 'POST':
        ...
        if request.FILES.get('fileinput'):
            user.foto_perfil = request.FILES['fileinput']
        elif 'remove_image' in request.POST:  # Este es el nombre del campo si decides usarlo
            user.foto_perfil = 'img/nofoto.jpg'  # Restablece a la imagen por defecto
            
        user.save()
        return redirect('mi_perfil')  # Redirige a la misma página después de guardar
