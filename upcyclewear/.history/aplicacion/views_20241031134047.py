...
def mi_perfil(request):
    ...
    if request.method == 'POST':
        ...
        if request.FILES.get('foto_perfil'):
            ...
            # Si no hay archivo, establecer la foto por defecto
        ...
        if current_password and new_password and renew_password:
            if new_password == renew_password:
                if user.check_password(current_password):  # Verificar la contraseña actual
                    user.set_password(new_password)  # Cambiar la contraseña
                    user.save()  # Guardar los cambios
                    messages.success(request, 'Tu contraseña ha sido cambiada con éxito. Por favor, vuelve a iniciar sesión.')
        ...
        return redirect('mi_perfil')  # Redirigir a la vista del perfil
