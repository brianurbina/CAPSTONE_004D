...
def actualizar_estado_reporte(request, reporte_id, nuevo_estado):
    reportes = get_object_or_404(ReporteUsuario, id=reporte_id)
    reportes.estado = nuevo_estado

    if request.method == 'POST':
        # Obtener el resultado del cuerpo de la solicitud
        data = json.loads(request.body)
        resultado = data.get('resultado')

        # Aquí puedes agregar el resultado al reporte o manejarlo como desees
        reportes.resultado = resultado  # Asegúrate de que tu modelo tenga este campo
        reportes.save()
