...
def finalizar_donacion2(request, id):
    if request.method == 'POST':
        ...
        try:
            ...
            # Obtener o crear la instancia de Peticion relacionada con la Donacion
            peticion = Peticion.objects.get_or_create(donacion=donacion)[0]  # O ajusta según tu relación

            # Crear una notificación para el receptor
            notificacion_id = uuid.uuid4()
            Notificacion.objects.create(
                id=notificacion_id,
                usuario=receptor,  # Asegurarse que 'usuario' es una instancia válida de 'Usuario'
                mensaje=f"El usuario {donacion.usuario.username} ha solicitado finalizar la donación: {donacion.titulo}.",
                leida=False,
                respuesta="PENDIENTE",
                peticion=peticion  # Relacionamos la notificación con la instancia de Peticion
            )
