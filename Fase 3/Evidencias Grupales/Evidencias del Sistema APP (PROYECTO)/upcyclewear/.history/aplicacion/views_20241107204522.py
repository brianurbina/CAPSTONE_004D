...
def fundaciones(request):
    query = request.GET.get('q')  # Obtener la búsqueda del usuario
    comuna_region = request.GET.get('comuna_region', 'TODOS')
    
    # Filtrar fundaciones que tienen el campo 'aprobada' en True
    foundations = Fundacion.objects.filter(aprobada=True)

    # Filtrar por comuna_region (en este caso, se filtra por comuna específica)
    if comuna_region != 'TODOS':
    ...
    # Obtener las opciones de comuna y región de tu modelo
    comuna_choices = Fundacion._meta.get_field('comuna_region').choices

    # Crear un formato adecuado para el template, un diccionario con regiones y comunas
    regiones_comunas = []
    for region, comunas in comuna_choices:
        regiones_comunas.append((region, comunas))
    
    context = {
        'foundations': page_obj,
        'query': query,
        'comuna_choices': regiones_comunas,  # Pasar las regiones y comunas al template
        'selected_comuna_region': comuna_region,  # El valor seleccionado para la comuna
    }
