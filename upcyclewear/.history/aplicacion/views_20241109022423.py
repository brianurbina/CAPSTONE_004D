...
def obtener_mensajes_no_leidos(request):
    if request.user.is_authenticated:
        ...
        # Obtener detalles de los mensajes (incluyendo el nombre de usuario y la foto de perfil del emisor)
        mensajes_detalles = mensajes_no_leidos[:3].values('id_mensaje', 'mensaje', 'emisor__username', 'emisor__foto_perfil', 'fecha_hora')

        # Retornar los detalles de los mensajes y el contador en formato JSON
        return JsonResponse({
