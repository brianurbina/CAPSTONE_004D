...
def mi_perfil(request):
    ...
    if request.method == 'POST':
        ...
        if form_type == 'profile':  # Actualizar el perfil
            ...
            # Obtener los sufijos de las URLs
            facebook_suffix = request.POST.get('facebook_suffix', '').strip()
            instagram_suffix = request.POST.get('instagram_suffix', '').strip()
            linkedin_suffix = request.POST.get('linkedin_suffix', '').strip()

            # Combina la URL base con el sufijo ingresado
            user.facebook = f'https://www.facebook.com/{facebook_suffix}' if facebook_suffix else ''
            user.instagram = f'https://www.instagram.com/{instagram_suffix}' if instagram_suffix else ''
            user.linkedin = f'https://www.linkedin.com/{linkedin_suffix}' if linkedin_suffix else ''

            # Manejo de la foto de perfil
            if request.FILES.get('foto_perfil'):
                user.foto_perfil = request.FILES['foto_perfil']
            else:
                user.foto_perfil = 'img/nofoto.jpg'  # O establecer una ruta a la imagen por defecto
