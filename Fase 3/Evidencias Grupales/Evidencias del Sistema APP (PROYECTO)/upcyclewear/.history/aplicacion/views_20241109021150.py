...
def obtener_mensajes_no_leidos(request):
    if request.user.is_authenticated:
        # Filtrar mensajes donde el usuario es el receptor (usuario1 o usuario2), y el mensaje no ha sido leído
        mensajes_no_leidos = Mensaje.objects.filter(
            Q(conversacion__usuario1=request.user, emisor__ne=request.user) |  # Recibe el mensaje de usuario2
            Q(conversacion__usuario2=request.user, emisor__ne=request.user),  # Recibe el mensaje de usuario1
            leido=False
        )
