...
@login_required
def finalizar_donacion(request, peticion_id):
    # Obtener la petición
    peticion = get_object_or_404(Peticion, id=peticion_id)
    
    # Verificar que la petición esté en estado "Activa"
    if peticion.estado != 'Activa':
        return JsonResponse({'error': 'La petición no está activa.'}, status=400)
    
    # Obtener el donador a través del parámetro recibido
    donador_username = request.POST.get('donador_username')
    donador = get_object_or_404(Usuario, username=donador_username)
    
    # Cambiar el estado de la petición a "Completada"
    peticion.estado = 'Completada'
    peticion.donador = donador
    peticion.save()

    # Crear la notificación para el donador
    notificacion = Notificacion(
        usuario=donador, 
        mensaje="¡Gracias por completar la donación!",
        leida=False
    )
    notificacion.save()

    # Devolver respuesta de éxito
    return JsonResponse({'success': True, 'message': 'Donación finalizada y notificación enviada.'})
