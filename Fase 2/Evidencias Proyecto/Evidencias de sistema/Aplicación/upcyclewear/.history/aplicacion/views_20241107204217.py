...
def fundaciones(request):
    query = request.GET.get('q')  # Obtener la búsqueda del usuario
    comuna_region = request.GET.get('comuna_region', 'TODOS')  # Obtener filtro por comuna_region

    # Filtrar fundaciones que están aprobadas
    ...
    # Si comuna_region no es 'TODOS', aplicar el filtro
    ...
    # Obtener las opciones de comuna_region para pasarlas al contexto (si las tienes predefinidas)
    comuna_choices = Fundacion._meta.get_field('comuna_region').choices  # Si 'comuna_region' es un campo de choices en el modelo

    # Pasar todos los contextos necesarios
    return render(request, 'menuuser/fundaciones.html', {
        'foundations': page_obj,
        'query': query,
        'comuna_choices': comuna_choices,  # Opciones para el filtro de comuna_region
        'selected_comuna_region': comuna_region,  # Comuna seleccionada
    })
