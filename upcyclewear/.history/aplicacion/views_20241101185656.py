...
@csrf_exempt
def enviar_correo_reporte(request, reporte_id):
    if request.method == 'POST':
        data = json.loads(request.body)
        ...
        # Obtener el reporte
        reporte = ReporteUsuario.objects.get(id=reporte_id)
        
        # Configurar el correo
        asunto = f"Reporte de Usuario - Estado: {estado}"
        mensaje = (
            f"Estimado {reporte.usuario_reportante.username},\n\n"
            f"Su reporte sobre el usuario {reporte.usuario_reportado.username} ha sido {estado.lower()}.\n"
            f"Motivo: {motivo}\n\n"
            "Gracias por ayudarnos a mejorar la comunidad.\n\n"
            "Atentamente,\nEl equipo de moderación"
        )
        destinatario = reporte.usuario_reportante.email
        
        # Enviar el correo
        send_mail(asunto, mensaje, 'upcyclewearcl@gmail.com', [destinatario])
