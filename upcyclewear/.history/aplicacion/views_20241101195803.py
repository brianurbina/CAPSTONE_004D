...
def actualizar_estado_reporte(request, reporte_id, nuevo_estado):
    if request.method == 'POST':
        try:
            ...
            reporte.motivo = motivo  # Asegúrate de que este campo exista en tu modelo
            reporte.save()

            # Añadir el correo electrónico del usuario reportante en la respuesta
            email_reportante = reporte.usuario_reportante.email
            
            return JsonResponse({'status': 'success', 'email': email_reportante})
        except Exception as e:
