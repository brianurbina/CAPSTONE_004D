...
def editar_perfil(request):
    if request.method == 'POST':
        ...
        elif 'remove_foto' in request.POST:  # Si se solicita eliminar la foto
            user.foto_perfil = None  # Esto significa que no se guarda nada, pero si se guarda, puede dejar el default.

        # Aquí puedes establecer una ruta por defecto si la foto está vacía
        if not user.foto_perfil:  # Si no hay foto establecida
            user.foto_perfil = 'img/nofoto.jpg'  # Establecer la imagen por defecto

        # Guardar otros campos del usuario
        user.first_name = request.POST.get('first_name')
        user.last_name = request.POST.get('last_name')
        user.descripcion = request.POST.get('about')
        user.direccion = request.POST.get('company')
        user.telefono = request.POST.get('job')
        user.email = request.POST.get('country')
        user.facebook = request.POST.get('facebook')
        user.instagram = request.POST.get('instagram')
        user.linkedin = request.POST.get('linkedin')

        user.save()
        return redirect('perfil')  # Redirigir a la página deseada

    return render(request, 'tu_template.html', {'user': request.user.userprofile})
